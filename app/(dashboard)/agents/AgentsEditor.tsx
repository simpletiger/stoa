...
export default function AgentsEditor({ initialContent, filePath }: AgentsEditorProps) {
        ...
        <div className="flex-1 p-6 overflow-auto prose prose-invert prose-sm max-w-none">
          <ReactMarkdown
            components={{
              ...
              th: ({ children }) => (
                <th className="text-left p-2 font-semibold text-white">{children}</th>
              ),
              td: ({ children }) => (
                <td className="p-2 text-foreground/90">{children}</td>
              ),
            }}
          >
            {content}

...
app.post('/api/file/read', async (req, res) => {
  try {
    ...
    const fullPath = path.join(CLAWD_DIR, filePath)
...
app.get('/api/files/list', async (req, res) => {
  try {
    const { dir = '' } = req.query
    const targetDir = path.join(CLAWD_DIR, dir as string)
...
app.get('/api/skills/list', async (req, res) => {
  try {
    ...
    const skillsDir = path.join(path.dirname(path.dirname(openclawPath)), 'lib', 'node_modules', 'openclaw', 'skills')
        ...
        .map(async (entry) => {
          ...
          try {
            ...
            return {
              name: entry.name,
              title,
              path: path.join('skills', entry.name),
              description
            }
          } catch {
            return {
